...
class ViewsCacheMetadataTest extends BrowserTestBase {
  ...
  public function testCacheMetadata() {
    ...
    // Create an admin user with access to view ui.
    $admin_user = $this->drupalCreateUser(['administer views']);
    $this->drupalLogin($admin_user);

    // Load the view in the UI to confirm the summary works.
    $view_url = Url::fromRoute('entity.view.edit_display_form', [
      'view' => $view_name,
      'display_id' => $display_name,
    ]);
    $this->drupalGet($view_url);
    $this->assertSession()->statusCodeEquals(200);
    // $this->assertSession()->pageTextContains('Always cache');
    $this->assertSession()->pageTextContains('1 tags');
    ...
    // Confirm the summary has been updated.
    $this->drupalGet($view_url);
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->pageTextContains('3 tags');
    ...
    $this->drupalGet($view_url);
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->pageTextContains('3 tags | 2 contexts');
    ...
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()
      ->pageTextContains('3 tags | 2 contexts | 10 min/10 min');

    $view = Views::getView($view_name);
    $element = $this->render($view, $display_name);
    $this->assertEquals(600, $element['#cache']['max-age'], 'The view is cached for 600 seconds.');

    // Clear out all of the config to test the default summary return.
    $view = Views::getView($view_name);
    $this->setCacheOptions($view->storage, $display_name, [
      'cache_tags' => [],
      'cache_tags_exclude' => [],
      'cache_tags_for_row' => FALSE,
      'cache_contexts' => [],
      'cache_contexts_exclude' => [],
      'results_lifespan' => -1,
      'results_lifespan_custom' => NULL,
      'output_lifespan' => -1,
      'output_lifespan_custom' => NULL,
    ]);
    $view->storage->save();
